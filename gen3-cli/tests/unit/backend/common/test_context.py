...
import tempfile
import pytest
from cot.backend.common.context import (
    Context,
    NoLevelFileError,
    NoRootFileError,
    NoTenantFoundError,
...
def test_context_basics():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(cmdb('product', 'config', 'solutions', 'environment', 'environment.json'), 'wt+') as f:
            json.dump(
                {
        ...
        with open(cmdb('product', 'config', 'solutions', 'environment', 'segment', 'segment.json'), 'wt+') as f:
            json.dump(
                {
                    "Name": "segment_name"
                },
        ...
        with open(cmdb('accounts', 'account-name-1', 'config', 'account.json'), 'wt+') as f:
            json.dump(
                {
                    "Name": "account-name-1"
                },
        ...
        with open(cmdb('accounts', 'account-name-2', 'config', 'account.json'), 'wt+') as f:
            json.dump(
                {
                    "Name": "account-name-2"
                },
        ...
        with pytest.raises(NoLevelFileError):
            ProductLevel(cmdb('accounts', 'account-name-1'), config=dict(account="account-name-1"))
